...
-- 굿스포츠에서 출판한 도서의 제목과 글자수(BYTE수)를 검색하시오.
SELECT bookname 제목, 
        LENGTH(bookname) 글자수
FROM book
...
-- 날짜, 시간 함수
-- 서점은 주문일로부터 10일 후 매출을 확정함. 각주문의 확정일자를 구하시오.
-- 날짜에서 일수를 더할 때: date + 일 수 
SELECT orderid 주문번호, 
    orderdate 주문일,
    orderdate + 10 확정
FROM orders
